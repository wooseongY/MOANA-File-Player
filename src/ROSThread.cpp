...
void ROSThread::ros_initialize(ros::NodeHandle &n)
{
  ...
  lidar_pub_ = nh_.advertise<sensor_msgs::PointCloud2>("/lidar", 10000);
  xband_pub_ = nh_.advertise<sensor_msgs::Image>("/xband_radar", 10000);
  wband_pub_ = nh_.advertise<sensor_msgs::Image>("/wband_radar", 10000);
  stereo_left_pub_ = nh_.advertise<sensor_msgs::Image>("/camera_left", 10000); // camera_0
  stereo_right_pub_ = nh_.advertise<sensor_msgs::Image>("/camera_right", 10000); // camera_1
  ...
  while(1){
    ...
    while(!wband_thread_.data_queue_.empty())
    {
      ...
      if( to_string(data)+".png" == wband_next_.first && !wband_next_.second.empty() )
      {
        ...
        wband_out_msg.header.stamp.fromNSec(data);
        ...
        if(save_flag_ == true && process_flag_ == true){
          std::lock_guard<std::mutex> lock(bag_mutex_);
          sensor_msgs::ImagePtr image_msg = wband_out_msg.toImageMsg();
          bag_.write("/wband_radar", wband_out_msg.header.stamp, *image_msg);
        }
  ...
  while(1){
    ...
    while(!xband_thread_.data_queue_.empty()){
      ...
      if(to_string(data)+".png" == xband_next_.first && !xband_next_.second.empty()){
        ...
        xband_out_msg.header.stamp.fromNSec(data);
        ...
        if(save_flag_ == true && process_flag_ == true){
          std::lock_guard<std::mutex> lock(bag_mutex_);
          sensor_msgs::ImagePtr image_msg = xband_out_msg.toImageMsg();
          bag_.write("/xband_radar", xband_out_msg.header.stamp, *image_msg);
        }
  ...
  while(1){
    ...
    while(!stereo_left_thread_.data_queue_.empty()){
      ...
      if(to_string(data)+".jpg" == stereo_left_next_img_.first && !stereo_left_next_img_.second.empty()){
        ...
        if(save_flag_ == true && process_flag_ == true){
          std::lock_guard<std::mutex> lock(bag_mutex_);
          sensor_msgs::ImagePtr image_msg_left = left_out_msg.toImageMsg();
          bag_.write("/camera_left", left_out_msg.header.stamp, *image_msg_left);
        }
  ...
  while(1){
    ...
    while(!stereo_right_thread_.data_queue_.empty()){
      ...
      if(to_string(data)+".jpg" == stereo_right_next_img_.first && !stereo_right_next_img_.second.empty()){
        ...
        if(save_flag_ == true && process_flag_ == true){
          std::lock_guard<std::mutex> lock(bag_mutex_);
          sensor_msgs::ImagePtr image_msg_right = right_out_msg.toImageMsg();

          bag_.write("/camera_right", right_out_msg.header.stamp, *image_msg_right);
        }
